...
export function ActionSheet({
  ...
  return (
    <Sheet
      ...
      title={
        entry
          ? `${STRINGS.token.hi} #${entry.token_number} · ${entry.name || "—"}`
          : undefined
      }
    >
      {entry && (
        <div className="grid gap-2.5">
          {canArrive && (
            <button
              className="btn-primary h-touch w-full"
              onClick={() => onArrived(entry)}
            >
              ✓ {STRINGS.markArrived.hi}
            </button>
          )}
          <button
            className="btn-ghost h-touch w-full"
            onClick={() => onCallNow(entry)}
          >
            ▶ {STRINGS.callNow.hi}
          </button>
          <button
